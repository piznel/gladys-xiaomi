...
function createOne(device) {
  shared.errorDevices.delete(device.sid)
  shared.unknownDevices.delete(device.sid)
...
function getParameters() {
  return Promise.all([
    getDebugMode().catch(() => { return sails.log.info('Xiaomi module: debug mode inactive!') }),
    getDebugDevice().catch(() => { return sails.log.info('Xiaomi module: debug device mode inactive!') }),
    getPassword().catch(() => { return sails.log.info('Xiaomi module: No registered gateway password!') })
  ])
}
...
function getDebugMode() {
  return gladys.param.getValue('Xiaomi_debug')
    .then((debug) => {
      ...
      sails.log.info(`Xiaomi module : debug mode '${debug}' active!`)
...
function getDebugDevice() {
  return gladys.param.getValue('Xiaomi_debug_device')
    .then((debugDevice) => {
      shared.debugDevice = debugDevice;
      sails.log.info(`Xiaomi module: debug device '${debugDevice}' mode active!`)
      return Promise.resolve()
    })
