...
module.exports = class Xiaomi extends events.EventEmitter {
  ...
  _onMessage(msg, rsinfo) {
    ...
    // *********DEBUG START*******************
    if (message.model === shared.debugDevice && shared.debugDevice.length > 0 ) sails.log('Xiaomi _onMessage.msg :', message)
    ...
    if (!this.ready) {
      ...
      // *********DEBUG START*******************
      if ((shared.debug > 1 && shared.debug < 4) || shared.debug === 9) sails.log('Xiaomi devices :', this.devices)
      if ((shared.debug > 1 && shared.debug < 4) || shared.debug === 9) sails.log('Xiaomi shared.gateways :', shared.gateways)
      if (shared.debug === 3 || shared.debug === 9) sails.log('Xiaomi this._readAck :', this._readAck)
      // *********DEBUG END******************* 
    }
  }
            ...
            if (!status) {
              if (typeof(this.devices.get(sid)) !== 'undefined' && typeof(this.devices.get(sid).ip !== 'undefined')) {
                ...
                if (shared.debug === 6 || shared.debug === 9) {
                    sails.log('Xiaomi resend msg :', msg)
                    sails.log('Xiaomi this._resendReadCount :', this._resendReadCount)
                }
                // *********DEBUG END******************* 
