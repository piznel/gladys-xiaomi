...
module.exports = class Xiaomi extends events.EventEmitter {
  ...
  constructor() {
    ...
    this._readAck = new Map(); // contains the sid of the devices and gateway(s) (the key's map) with'true' if they have answered the command "read", otherwise 'false' (the value's map)
  ...
  _onMessage(msg, rsinfo) {
    ...
    // *********DEBUG START*******************
    if (message.model === shared.debugDevice && shared.debugDevice.length > 0 ) sails.log.debug('Xiaomi _onMessage.msg :', message)
    if ((shared.debug > 0 && shared.debug < 4) || shared.debug === 9) {
      sails.log.debug('Xiaomi _onMessage.msg :', message)
      sails.log.debug('Xiaomi _onMessage.rsinfo :', rsinfo)
    }

    if(message.sid === shared.logUnknown.sid) {
      gladys.socket.emit('xiaomi_module_log', message)
    }
    // *********DEBUG END*******************
    ...
    this._findDeviceMessage(message, ip)
      ...
      .catch(() => {
        sails.log.error('Xiaomi-home : new device ', message)
      });
    ...
    if (!this.ready) {
      switch (message.cmd) {
        ...
        case 'get_id_list_ack':
          ...
          sids.forEach((sid) => {
            if (!this._readAck.has(sid)) {
              this._readAck.set(sid, false);
            } else {
              shared.errorDevices.set(sid, 'NO ANSWER AT READ COMMAND')
            }
        ...
        case 'read_ack':
          ...
          this._readAck.set(message.sid, true);
      ...
      // everything is ready!
      if (this._devicesRead || this._resendReadCount === 5) {
        return Promise.each(shared.errorDevices.keys(), (sid) => {
          return Promise.resolve(this.devices.delete(sid))
        })
        .then(() => {
          this.ready = true;
          this.emit('ready');
      ...
      // *********DEBUG START*******************
      if ((shared.debug > 1 && shared.debug < 4) || shared.debug === 9) sails.log.debug('Xiaomi devices :', this.devices)
      if ((shared.debug > 1 && shared.debug < 4) || shared.debug === 9) sails.log.debug('Xiaomi shared.gateways :', shared.gateways)
      if (shared.debug === 3 || shared.debug === 9) sails.log.debug('Xiaomi this._readAck :', this._readAck)
      // *********DEBUG END******************* 
    }
  }
  ...
  _findDeviceMessage(message, ip) {
    if (this.devices.has(message.sid)) {
      ...
      return this._createDevice(message, ip)
        ...
        .catch((err) => {
          return Promise.reject('Xiaomi-home : unknown message')
        })
    }
  }
  ...
  _resendRead() {
    ...
    Promise.delay(2000).then(() => {
      ...
      if (this._readAck.size > 0 && this.devices.size > 0) {
        // if we aren't yet ready ...
        if (!this.ready && this._resendReadCount < 5) {
          for (let [sid, status] of this._readAck) {
            if (!status) {
              if (typeof(this.devices.get(sid)) !== 'undefined' && typeof(this.devices.get(sid).ip !== 'undefined')) {
                ...
                // *********DEBUG START*******************
                if (shared.debug === 6 || shared.debug === 9) {
                    sails.log.debug('Xiaomi resend msg :', msg)
                    sails.log.debug('Xiaomi this._resendReadCount :', this._resendReadCount)
                }
                // *********DEBUG END******************* 
  ...
  _testRead() {
    for (let result of this._readAck.values()) {
