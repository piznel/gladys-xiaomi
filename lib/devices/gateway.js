...
module.exports = class Gateway extends GenericDevice {
  ...
  deviceMessage(message, ip) {
    ...
    switch (message.cmd) {
      case 'get_id_list_ack':
        ...
        sids.forEach((sid) => {
          setTimeout(() => {
            ...
            // *********DEBUG START*******************
            if (shared.debug === 9) sails.log.debug('Xiaomi gateway read :', msg)
            // *********DEBUG END******************* 
      ...
      case 'read_ack':
        ...
        break;
      case 'write_ack':
        const dataWrite = JSON.parse(message.data);
        if (dataWrite.error === 'Invalid key') {
  ...
  sendCommand(model, sid, cmd) {
    ...
    if (typeof(this._password) !== 'undefined' && this._password.length !== 0) {
      ...
      // *********DEBUG START*******************
      if (shared.debug === 7 || shared.debug === 9) sails.log.debug('Xiaomi gateway.sendCommand :', msg)
      // *********DEBUG END*******************
  ...
  // ---------- private methods ----------
  _calculateKey() {
    let tmpPassword = 'abcdefghijklmnop'
    if (typeof(this._password) === 'undefined' || this._password.length === 16) tmpPassword = this._password
    const cipher = crypto.createCipheriv('aes-128-cbc', tmpPassword, AQARA_IV);
