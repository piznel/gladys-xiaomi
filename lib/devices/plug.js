      ...
      if (data.status) {
        if (data.status !== 'unknow') {
          ...
          if(data.status === 'off') {
            ...
            eventdata.instantaneous_power = 0
          }
        }
      }
  ...
  deviceExec(deviceType, state) {
    if (deviceType.identifier === this.sid) {
      if (typeof(state) !== 'undefined' && state.value !== null) {
        ...
        return Promise.reject(`Xiaomi Module : invalid status for plug identifier whose identifier is "${this.sid}"`)
      }
    } else {
      return Promise.reject(`Xiaomi Module : the sid "${this.sid}" is not that of a plug.`)
    }
  }
}