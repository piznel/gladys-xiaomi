...
module.exports = function createDeviceState(device) {
    for (let _attribute in device) {
        ...
        switch (_attribute) {
            ...
            case 'Channel_0':           // wired switch single and duplex 
            case 'Channel_1':           // wired switch duplex
        ...
        //console.error('New state  = ' + device.model + ' ' + device.sid + ', _devicetype = ' + _devicetype + ', value = ' + device[_attribute] + ', _value = ' + _value);
        if (_value || _value === 0) {
