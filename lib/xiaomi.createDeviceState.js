...
module.exports = function createDeviceState(device) {
  for (let _attribute in device) {
    ...
    // *********DEBUG START*******************
    if (shared.debug === 5 || shared.debug === 9) sails.log('Xiaomi createDeviceState :' + device.model + ' ' + device.sid + ', _devicetype = ' + _devicetype + ', value = ' + device[_attribute] + ', _value = ' + _value);
    // *********DEBUG END*******************
