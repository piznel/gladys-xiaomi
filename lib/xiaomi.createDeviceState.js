...
module.exports = function createDeviceState(device) {
  for (let _attribute in device) {
    ...
    // *********DEBUG START*******************
    if (shared.debug === 5 || shared.debug === 10) sails.log.debug('Xiaomi createDeviceState :' + device.model + ' ' + device.sid + ', _devicetype = ' + _devicetype + ', value = ' + device[_attribute] + ', _value = ' + _value);
    // *********DEBUG END*******************
    if (_value || _value === 0) {
      return gladys.deviceType.getByIdentifier({ deviceIdentifier: device.sid, deviceService: 'xiaomi-home', deviceTypeIdentifier: _devicetype })
        .then((deviceType) => {
          return gladys.deviceState.create({ devicetype: deviceType.id, value: _value })
            .then((deviceState) => {
