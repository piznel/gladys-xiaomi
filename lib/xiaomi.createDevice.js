...
module.exports = function createDevice(device) {
  ...
  switch (device.model) {
    ...
    case 'Single_Wired_Switch_N':
      newDevice = {
        device: {
          name: 'Single Wired Switch Neutral',
          identifier: device.sid,
          protocol: 'zigbee',
          service: 'xiaomi-home'
        },
    ...
    case 'Duplex_Wired_Switch_N':
      newDevice = {
        device: {
          name: 'Duplex Wired Switch Neutral',
          identifier: device.sid,
          protocol: 'zigbee',
          service: 'xiaomi-home'
        },
  ...
  if (newDevice) {
    ...
    // *********DEBUG START*******************
    if (shared.debug === 4 || shared.debug === 9) sails.log('Xiaomi createDevice.newDevice :', newDevice)
    // *********DEBUG END*******************
    ...
    gladys.device.create(newDevice)
      .then((data) => {
        ...
        // *********DEBUG START*******************
        if (shared.debug === 4 || shared.debug === 9) sails.log('Xiaomi createDevice.gladys :', data.device.name + ', id :' + data.device.identifier)
        // *********DEBUG END*******************
